...
def upgrade_database_schema():
    ...
    try:
        ...
        cursor.execute("ALTER TABLE files ADD COLUMN IF NOT EXISTS owner_id BIGINT NOT NULL;")
...
def auto_clean_trash():
    ...
    cursor = conn.cursor()
    try:
        # Automatically deletes files that have been in the trash for 30 days
...
def save_to_cloud(file_id, file_name, file_type, owner_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    new_code = ''.join(random.choices(string.ascii_letters + string.digits, k=6))
    
    cursor.execute('''
        INSERT INTO files (file_id, file_name, file_type, owner_id, short_code)
        VALUES (%s, %s, %s, %s, %s)
    ''', (file_id, file_name, file_type, owner_id, new_code))
    conn.commit()
    cursor.close()
    conn.close()
...
class DummyHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Cloud Server is awake!")
...
async def post_init(application: Application):
    ...
    logging.info("Database upgraded and bot started successfully.")
...
def get_main_menu():
    keyboard = [
        [InlineKeyboardButton("🔥 Most Downloaded", callback_data='p_tp_1_none'),
...
# ==========================================
#     PAGINATION & STRICT PRIVACY ENGINE
# ==========================================
def get_paginated_results(f_type, f_param, page, user_id):
    ...
    # STRICT PRIVACY: Every query is locked to the user's specific Telegram ID.
    ...
    if f_type == 'rc':
        ...
        cursor.execute(f"SELECT short_code, file_id, file_name, file_type FROM files WHERE deleted_at IS NULL {auth_filter} ORDER BY download_count DESC LIMIT %s OFFSET %s", (limit, offset))
        header = f"🔥 **Most Downloaded (Page {page}):**\n\n"
    elif f_type == 'ct':
...
def format_results_msg(results, header, is_trash):
    ...
    for row in results:
        if is_trash:
            msg += f"📁 `{row[2]}`\n♻️ /restore_{row[0]} | ☢️ /pdel_{row[0]}\n\n"
        else:
            msg += f"📁 `{row[2]}` ({row[3]})\n"
            msg += f"👁️ /view_{row[0]} | 📥 /get_{row[0]}\n"
            msg += f"🔗 /share_{row[0]} | 🗑️ /del_{row[0]}\n\n"
    return msg

def is_authorized_to_modify(short_code, user_id):
    # Strictly checks if the file owner matches the Telegram User ID requesting modification
    conn = get_db_connection()
    cursor = conn.cursor()
...
async def show_main_menu_msg(update: Update, user_id):
    welcome_text = (
        ...
        "• `/view_CODE` - Preview the image/video.\n"
        "• `/get_CODE` - Download the original file.\n"
        "• `/share_CODE` - Generate a shared link for others!\n"
        "• `/rename_CODE New Name` - Change file name.\n"
        "• `/del_CODE` - Move file to Recycle Bin.\n\n"
        "🔒 *Note: Your vault is 100% private. Other users cannot see your files.*\n\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].startswith('get_'):
        if not is_registered(user_id):
            await update.message.reply_text(
                ...
                "*(Once registered, just click the shared link again!)*",
                parse_mode='Markdown'
            )
            return
            
        short_code = context.args[0].split('_')[1]
        conn = get_db_connection()
        cursor = conn.cursor()
    ...
    if not is_registered(user_id):
        await update.message.reply_text("👋 **Welcome to the Cloud!**\n\nTo get started, please **reply with your Full Name** to register.")
        return
...
async def share_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        cmd_parts = update.message.text.split('_')
        if len(cmd_parts) < 2: return
        short_code = cmd_parts[1]
        share_link = f"https://t.me/{context.bot.username}?start=get_{short_code}"
...
async def delete_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not is_authorized_to_modify(short_code, user_id):
            await update.message.reply_text("🚫 Access Denied: You cannot delete this file.")
            return
...
async def restore_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        cursor.close()
        conn.close()
        if result: await update.message.reply_text(f"♻️ **Restored:** `{result[0]}`")
    except Exception as e: await update.message.reply_text(f"❌ Error: {str(e)}")
...
async def perm_delete_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        cursor.close()
        conn.close()
        if result: await update.message.reply_text(f"☢️ **Permanently Deleted:** `{result[0]}`")
    except Exception as e: await update.message.reply_text(f"❌ Error: {str(e)}")

async def rename_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        match = re.match(r'^/rename_(\w+)\s+(.+)', update.message.text)
        if match:
            short_code = match.group(1)
            user_id = update.effective_user.id
            if not is_authorized_to_modify(short_code, user_id):
                await update.message.reply_text("🚫 You cannot edit other users' files.")
                return
        ...
        else: await update.message.reply_text("⚠️ Please type the new name after the command. Example: `/rename_aBcD12 My Photo`")
    except Exception as e: await update.message.reply_text(f"❌ Error: {str(e)}")
...
async def get_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if len(cmd_parts) < 2: return
        short_code = cmd_parts[1]
        if not is_authorized_to_modify(short_code, update.effective_user.id): return
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE files SET download_count = download_count + 1 WHERE short_code = %s RETURNING file_id", (short_code,))
        result = cursor.fetchone()
...
# ==========================================
#     INTERACTION HANDLERS (Buttons, Search, Upload)
# ==========================================
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'menu_main':
        ...
        await query.edit_message_text("📂 **Select a Category:**", reply_markup=get_categories_menu())
    elif data == 'menu_dates':
        await query.edit_message_text("📅 **Select Timeframe Type:**", reply_markup=get_dates_menu())
    elif data == 'menu_weeks':
        await query.edit_message_text("📆 **Select Weeks Filter:**", reply_markup=get_weeks_menu())
    elif data == 'menu_months':
        await query.edit_message_text("📆 **Select Months Filter:**", reply_markup=get_months_menu())
    elif data == 'menu_trash':
        await query.edit_message_text("🗑️ **Recycle Bin:**\nFiles stay here for 30 days.", reply_markup=get_trash_menu())
    elif data == 'menu_trash_cat':
        await query.edit_message_text("📂 **Trashed Categories:**", reply_markup=get_trash_categories_menu())
        ...
        cursor.close()
        conn.close()
        await query.edit_message_text(f"☢️ **Trash Emptied!**\n{deleted_count} files were permanently deleted.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back to Main", callback_data='menu_main')]]))
        ...
        await query.edit_message_text("🗂️ **Folder Hashtags:**", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def handle_text_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_registered(user_id):
        register_user(user_id, search_query)
        await update.message.reply_text(f"✅ Registered as **{search_query}**! You can now use the vault.")
        await show_main_menu_msg(update, user_id)
        return
        
    res, has_next, head, target, trash = get_paginated_results('sr', search_query[:30], 1, user_id)
    await update.message.reply_text(text=format_results_msg(res, head, trash), reply_markup=build_pagination_keyboard('sr', search_query[:30], 1, has_next, target))

async def handle_files(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not is_registered(user_id):
        await update.message.reply_text("⚠️ Please reply with your Full Name to register before uploading!")
        return
    ...
    fid, ftype, orig_name = None, "Other", None
    ...
    if fid:
        ...
        save_to_cloud(fid, final_name, ftype, user_id)
        await update.message.reply_text(f"✅ Saved: `{final_name}`")
...
if __name__ == '__main__':
    ...
    app.add_handler(MessageHandler(filters.Regex(r'^/share_\w+'), share_file))
    app.add_handler(MessageHandler(filters.Regex(r'^/del_\w+'), delete_file))
    app.add_handler(MessageHandler(filters.Regex(r'^/get_\w+'), get_file))
    app.add_handler(MessageHandler(filters.Regex(r'^/view_\w+'), view_file))
    app.add_handler(MessageHandler(filters.Regex(r'^/restore_\w+'), restore_file))
    app.add_handler(MessageHandler(filters.Regex(r'^/pdel_\w+'), perm_delete_file))
    app.add_handler(MessageHandler(filters.Regex(r'^/rename_\w+'), rename_file))
